...
<div class="container">
    ...
    <div class="category-note-container">
        <div v-if="state.isNull" class="no-data">
            <div class="tishi">Sorry~</div>
            <div class="tishi">没有<span style="background-color:red">{{ Route.query.key}}</span>相关内容</div>
        ...
        <div v-for="(item,index) in state.noteList" :key="index" class="note-item" @click="goDetail(item.id)">
               ...
               <div v-html="item.content" class="note-content" style="overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;"></div>
            </div>
        </div>
      <!-- 分页器       -->
      <el-pagination 
...
import { getNoteByTitle } from '../../api/note';
...
const state=reactive({
    ...
    pageSize:3,
    total:0,
    noticeText:`您搜索的标题为:${Route.query.key}`,
    isNull:0
})


function initData(pageindex,pagesize){
    getNoteByTitle(Route.query.key).then(res=>{
        let  initList =res.data
        state.total =initList.length
        if(initList.length==0){
            state.isNull=1
        }
        let fenyeList = initList.slice((pageindex-1) * pagesize,(pageindex-1) * pagesize + pagesize)
